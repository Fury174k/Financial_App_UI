...
export default function Expenses({ className }) {
  ...
  const fetchData = async (cacheKey) => {
    ...
    try {
      ...
      const budgetResponse = await fetch("https://financial-tracker-api-1wlt.onrender.com/api/budget/current/", {
      ...
      // Fetch transactions
      const transactionsResponse = await fetch("https://financial-tracker-api-1wlt.onrender.com/api/transactions/", {
        headers: { Authorization: `Token ${token}` }
      });
      let totalExpenses = 0;
