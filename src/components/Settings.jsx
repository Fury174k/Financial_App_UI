...
export default function Settings() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch accounts
        const accRes = await fetch("https://financial-tracker-api-1wlt.onrender.com/api/accounts/", {
        ...
        const profRes = await fetch("https://financial-tracker-api-1wlt.onrender.com/api/profile/", {
          headers: { Authorization: `Token ${authToken}` },
        });
  ...
  const handleSave = async (type, id) => {
    ...
    try {
      ...
      let body = {};
      if (type === "account") {
        url = `https://financial-tracker-api-1wlt.onrender.com/api/accounts/${id}/`;
        body = editing[`account-${id}`];
      } else if (type === "profile") {
        url = `https://financial-tracker-api-1wlt.onrender.com/api/profile/`;
  ...
  const confirmDeleteAccount = async () => {
    setLoading(true);
    try {
      const res = await fetch(`https://financial-tracker-api-1wlt.onrender.com/api/accounts/${showDeleteConfirm}/`, {
        method: "DELETE",
        headers: { Authorization: `Token ${authToken}` },
      });
      if (res.ok) {
